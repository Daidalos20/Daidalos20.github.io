const item_containers = {
  ...
  },
  {
    ...
    "location": "G4(oben mitte)+G4(unten links am Schnellangriffsverteiler);9+1=10"
  },
  {
    "name": "5m B-Schlauch",
  ...
  },
  {
    ...
    "location": "G3(mitte rechts, von Rechts ans Blech befestigt, \u00fcberm Feuerl\u00f6scher);1"
  },
  {
  ...
  },
  {
    ...
    "location": "G3(unten rechts, zw. Feuerl\u00f6scher und K\u00fcbelspritze)"
  }
]
};
...
const next_stop_button_element = document.getElementById("next_stop_button");
...
function updateTimer() {
  let seconds = timer_time_ms / 1000; // divide by 10 to get 2 decimal places
  let timerElement = document.getElementById('timer');
  timerElement.textContent = `Gesammt Zeit ${seconds.toFixed(2)}s`; // display 2 decimal places
}
...
function startTimer() {
  if (!timer_running) {
    ...
    intervalId = setInterval(() => {
      timer_time_ms+=100;
      updateTimer();
    }, 100); // update every 100ms to get 1 decimal place
  }
}
...
function startNewRound(container_name){
  ...
  next_stop_button_element.innerHTML = `Neue Runde mit ${current_container_name} starten!`;
...
function loadContainer(container_name) {
  items_left = shuffleList(item_containers[container_name]);
...
function start_stop_timer() {
  if (!timer_running) {
    ...
    if (items_left.length === 0) {
      ...
      show_next_item();
      next_stop_button_element.innerHTML = "Gefunden!";
      startTimer();
    }
  } else {
    // item found
    ...
    if (items_left.length === 0){
      ...
      next_stop_button_element.innerHTML = "Nächster Gegenstand";
    }
  }
}

function show_results() {
  itemElement.innerHTML = "Alle Gegenstände zugeordnet";
  
  const nextStopButtonElement = document.getElementById("next_stop_button");
  nextStopButtonElement.innerHTML = `Erneute Runde mit ${current_container_name} Starten
?</br>Alternativ:Oben rechts können andere Orte beübt werden.`;
}

function update_progress_bar(){
  const progressBarElement = document.getElementById("progress-bar");
  const progressTextElement = document.getElementById("progress-text");
  const items_done = (item_containers[current_container_name].length -items_left.length)
  const progress = (items_done / item_containers[current_container_name].length) * 100;
  progressBarElement.style.width = `${progress}%`;
  progressTextElement.textContent = `${items_done} von ${item_containers[current_container_name].length}`;
}

function show_next_item() {
  let item = items_left.pop();
  const itemElement = document.getElementById("item");
  itemElement.innerHTML = item.name;
  // Set location hint
  const locationHintElement = document.getElementById("location-hint");
  locationHintElement.innerHTML = item.location;
